#!/usr/bin/env python3
"""
DRND_ST_LENSING_FAIR_NFW_TEST.py

Fair-play control test:
- DRND fixed a0
- DRND fixed a0 + screened environment
- NFW per-bin
- NFW with global concentration
- NFW with concentration fixed by a simple mass-concentration relation
- baryon baselines

Usage:
  python DRND_ST_LENSING_FAIR_NFW_TEST.py ^
    --lensing-csv lensing_data.csv ^
    --bin-mode auto ^
    --n-bins 4 ^
    --outdir DRND_ST_FAIR_NFW_TEST
"""
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        for row in csv.DictReader(f):
            ...
            if np.isfinite(R) and np.isfinite(ds) and np.isfinite(err) and R > 0 and ds > 0 and err > 0:
                rows.append({"lens_bin_original": lb, "R_kpc": float(R), "DeltaSigma": float(ds), "DeltaSigma_err": float(err), "logMbar_input": float(logM) if np.isfinite(logM) else np.nan})
    if not rows:
...
def fit_case(case, bins, data, args, a0):
    ...
    def obj(p):
        return float(np.sum(residuals(case, p, bins, data, args, a0) ** 2))
    ...
    k = len(opt.x)
    dof = max(n - k, 1)
    return {"case": case, "chi2": chi2, "n_data": n, "k_params": k, "dof": dof, "chi2_per_dof": chi2 / dof, "chi2_per_data": chi2 / max(n, 1), "AIC": chi2 + 2 * k, "BIC": chi2 + k * math.log(max(n, 2)), "success": bool(opt.success), "message": str(opt.message), "pred_rows": pred_rows, "bin_rows": bin_rows}
...
def main():
    ...
    ap.add_argument("--outdir", default="DRND_ST_FAIR_NFW_TEST")
    args = ap.parse_args()
    ...
    for metric in ["BIC", "AIC"]:
        ...
        ax.set_title(f"DRND-ST fair NFW comparison by {metric}")
    ...
    summary = {
        "model": "DRND-ST fair NFW parameter-control test",
    ...
    print(json.dumps(safe_json({"outdir": str(outdir), "decision": decision, "files": ["summary.json", "tables/model_comparison.csv", "tables/bin_parameters.csv", "plots/model_comparison_bic.png", "plots/model_comparison_aic.png"]}), indent=2))
